...
# Inicializa la ventana principal para el inicio de sesión
def login_window():
    login = Tk()
    ...
    # Añade padding a la ventana  
    login.configure(padx=20, pady=20)  

    # Variables de entrada para el nombre de usuario y la contraseña
    username = StringVar()
    password = StringVar()
    ...
    def login_user():
        ...
        username_input = username.get()
         # Hashea la contraseña ingresada utilizando SHA-256
        ...
        if result:
            # Cierra la ventana de inicio de sesión si las credenciales son válidas
            login.destroy()
            # Llama a la función para abrir la ventana principal  
            main_window()  
        else:
    ...
    Button(login, text="Iniciar Sesión", command=login_user).grid(row=2, column=0, columnspan=2, padx=10, pady=10)
...
def main_window():
    ...
    def abrirVentanaCargos():
        ...
        ventanaCargos.resizable(False, False)  # Deshabilitar redimensionamiento de la ventana
        ventanaCargos.configure(padx=20, pady=20)
        ...
        lblCargo.grid(row=0, column=0, padx=10, pady=10)
        txtCargo = Entry(ventanaCargos, textvariable=cargo)
        txtCargo.grid(row=0, column=1, padx=10, pady=10)
    ...
    def abrirVentanaContactosEmergencia():
        ...
        ventanaContactosEmergencia.resizable(False, False) 
        ventanaContactosEmergencia.configure(padx=20, pady=20)
